// early_confirm_bt.cpp — S-2026-07-20 EARLY-CONFIRM certification (operator order:
// "stupid to wait an hour if the next BE is met earlier — fix this now; check it via
// a backtest").
//
// Question: does firing the BE-ENTRY confirm INTRA-BAR (the moment the mark covers
// confirm) instead of at the H1 close change the certified economics of the live
// BECASC mimic cells? Operator thesis: it cannot hurt — the BE floor exits a
// reversal anyway.
//
// Method: same harness family as the S-18ab/ag/ai lowthr certs
// (eth_ujmimic_15_becascade_bt.cpp) driving the REAL live MimicLadderCompanion from
// /Users/jo/ChimeraCrypto/include (which now carries intrabar_confirm_opens_).
//   UM_EARLY=0  -> byte-original drive: stop_check_only(low) + observe(close). The
//                  close-only confirm baseline (certified behavior).
//   UM_EARLY=1  -> live-faithful tick sequence per bar: observe(high) FIRST (this
//                  finalizes the prior bar's close-walk exactly as live's first tick
//                  after the boundary does, then runs the intra-bar confirm fill at
//                  the bar high = PESSIMISTIC fill, real tick fill is at the confirm
//                  level <= high), then stop_check_only(low) (worse-of: the same
//                  bar's low tests the freshly opened leg's floor/stop), then
//                  observe(close).
// Cost RT=30/60bp (stricter than measured 28), confirm60 BE-ENTRY anchored
// (le=epx floored-on-open), legs8 — the live rollout spec.
//
// Build: clang++ -std=c++17 -O2 -I/Users/jo/ChimeraCrypto/include early_confirm_bt.cpp -o early_confirm_bt
...
static Agg run(const std::vector<Bar>& b, int W, double thr, double g, int legs, double rt){
    ...
    c.parent_tag="SELF"; c.tag="EC-BT"; c.symbol="btusdt";
...
int main(){
    ...
    std::printf("EARLY-CONFIRM CERT — coin=%s early=%d bars=%zu thr=%.2f%% confirm=%.0fbp anchor=%d RT=%.0f/%.0fbp\n",
