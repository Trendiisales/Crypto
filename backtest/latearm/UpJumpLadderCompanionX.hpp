...
public:
    ...
    struct Config {
        ...
        int     det_w         = 0;
        double  det_thr       = 0.0;
        // ── LATE-ARM FIX CANDIDATES (S-2026-07-07 item 3, ADA -244bp incident) ──
        // All gate the leg OPEN (the +be_bp trip, incl. reclip re-opens). 0 = OFF =
        // byte-identical to the live engine. Event context = the self-detected window.
        int     open_cutoff_bars = 0;   // (a) no NEW open if bars-since-event-entry > N
        int     open_expand_k    = 0;   // (b) open only if event made a new high within K bars
        double  open_max_peak_gb = 0.0; // (c) no open if giveback from event peak MFE > frac (0..1)
    };
...
private:
    ...
    void observe_be_(bool parent_in_pos, double parent_entry_px, double cur_px, int64_t ts_ms, int64_t bar) {
        ...
        if (entry_ref_ != parent_entry_px) {
            ...
            reset_session_();
            entry_ref_ = parent_entry_px;
            ev_entry_bar_ = bar; ev_hwm_ = cur_px; ev_hwm_bar_ = bar;   // late-arm gates: event context
            legs_.push_back(make_be_leg_("T1", parent_entry_px, cfg_.tight));
            legs_.push_back(make_be_leg_("T2", parent_entry_px, cfg_.wide));
        }
        if (cur_px > ev_hwm_) { ev_hwm_ = cur_px; ev_hwm_bar_ = bar; }  // late-arm gates: event high-water
        last_be_px_ = cur_px;
    ...
    void step_be_(Leg& lg, double cur, int64_t ts, int64_t bar) {
        if (!lg.open) {                                     // FLAT: wait for +be_bp from ref -> open here
            ...
            // ── LATE-ARM FIX CANDIDATES: suppress a LATE/stale open (leg stays flat) ──
            if (cfg_.open_cutoff_bars > 0 && bar - ev_entry_bar_ > cfg_.open_cutoff_bars) return;   // (a)
            if (cfg_.open_expand_k    > 0 && bar - ev_hwm_bar_   > cfg_.open_expand_k)    return;   // (b)
            if (cfg_.open_max_peak_gb > 0.0 && entry_ref_ > 0.0 && ev_hwm_ > entry_ref_) {          // (c)
                const double peak_fav = ev_hwm_ / entry_ref_ - 1.0;
                const double cur_fav  = cur     / entry_ref_ - 1.0;
                if ((peak_fav - cur_fav) / peak_fav > cfg_.open_max_peak_gb) return;
            }
    ...
    // late-arm gate event context (be_floor)
    int64_t ev_entry_bar_ = 0;     // bar the current event's legs were seeded
    double  ev_hwm_       = 0.0;   // event high-water price
    int64_t ev_hwm_bar_   = 0;     // bar of the event high-water
};
