...
#include <iomanip>
#include <fstream>
#include <sys/stat.h>   // mkdir (POSIX; matches crypto-repo idiom, no std::filesystem link dep)
#include <algorithm>
...
public:
    struct Config {
        ...
        // CRYPTO port (SKIP_OMEGA=1): stall_accountant.py disables the empty-omega guard so the
        // harvest (incl. ENGINE_EXIT prune) ALWAYS runs, even with zero live rows. The Omega path
        // keeps the guard (a flat VPS book == restart blip -> skip). Set true for crypto books.
        bool   skip_empty_guard = false;
    };

    explicit StallBook(Config c) : cfg_(std::move(c)) {
        usd_mode_ = cfg_.arm_usd > 0.0;
        mkdir_p_(cfg_.dir);   // never silent-die on a missing state dir (POSIX mkdir -p)
    ...
    void step(const std::vector<StallLiveRow>& rows_all, int gold_bull, int64_t now) {
        ...
        const bool omega_empty = cfg_.skip_empty_guard ? false : rows_all.empty();
        if (!omega_empty) {
...
private:
    ...
    std::string mkkey_(const std::string& book, const std::string& eng,
        ...
        // trailing zeros (73.43 -> "73.43", NOT "%.4f"->"73.4300"). Using %.4f here orphaned
        // seed-migrated python positions across the cutover (key never matched the live harvest
        // -> spurious ENGINE_EXIT + reopen). pyfloat() reproduces str(round(x,4)) exactly.
    ...
    // POSIX mkdir -p (create each path component; ignore EEXIST). Absolute or relative.
    static void mkdir_p_(const std::string& path) {
